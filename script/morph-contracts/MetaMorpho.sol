    ...
    IMetaMorphoStaticTyping
{
    ...
    modifier onlyCuratorRole() {
        ...
        if (sender != curator && sender != owner())
            revert ErrorsLib.NotCuratorRole();
    ...
    modifier onlyGuardianRole() {
        if (_msgSender() != owner() && _msgSender() != guardian)
            revert ErrorsLib.NotGuardianRole();
    ...
    function setIsAllocator(
        ...
        if (isAllocator[newAllocator] == newIsAllocator)
    ...
    function setFee(uint256 newFee) external onlyOwner {
        ...
        if (newFee != 0 && feeRecipient == address(0))
            revert ErrorsLib.ZeroFeeRecipient();
    ...
    function setSupplyQueue(
        ...
        if (length > ConstantsLib.MAX_QUEUE_LENGTH)
            ...
            if (!seen[i]) {
                ...
                if (MORPHO.supplyShares(id, address(this)) != 0) {
                    ...
                    if (block.timestamp < config[id].removableAt) {
                        revert ErrorsLib.InvalidMarketRemovalTimelockNotElapsed(
                            id
                        );
                    }
                }
    ...
    function _checkTimelockBounds(uint256 newTimelock) internal pure {
        if (newTimelock > ConstantsLib.MAX_TIMELOCK)
            revert ErrorsLib.AboveMaxTimelock();
        if (newTimelock < ConstantsLib.MIN_TIMELOCK)
    ...
    function _supplyMorpho(uint256 assets) internal {
        for (uint256 i; i < supplyQueue.length; ++i) {
            ...
            uint256 toSupply = UtilsLib.min(
                supplyCap.zeroFloorSub(supplyAssets),
                assets
            );
    ...
    function _withdrawMorpho(uint256 assets) internal {
        for (uint256 i; i < withdrawQueue.length; ++i) {
            ...
            ) = _accruedSupplyBalance(marketParams, id);
    ...
    function _simulateWithdrawMorpho(
        uint256 assets
    ) internal view returns (uint256) {
        for (uint256 i; i < withdrawQueue.length; ++i) {
            Id id = withdrawQueue[i];
